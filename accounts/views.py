...
from feature.models import Notification
...
def book_photographer(request, photographer_id):
    ...
    if request.user.profile.role == 'photographer':
        messages.error(
            request,
            "Access Denied: Photographer accounts cannot book other photographers. Please log in as a Client."
        )
        return redirect('home')
    ...
    if has_active_booking:
        ...
        messages.warning(
            request,
            f"You already have an active booking request with {photographer.user.username}."
        )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if booking.event_date < timezone.now():
                ...
                messages.error(
                    request,
                    "Failed: Your event date and time must be in the future!"
                )

                return render(
                    request,
                    'accounts/book_photographer.html',
                    {
                        'form': form,
                        'photographer': photographer
                    }
            ...
            Notification.objects.create(
                ...
                message=f"You received a new booking request from {request.user.username}"
            )

            messages.success(
                request,
                f"Your booking request has been sent to {photographer.user.username}!"
            )
...
def update_booking_status(request, booking_id, status):
    ...
    booking = get_object_or_404(
        Booking,
        id=booking_id,
        photographer__user=request.user
    )

    if status in ['accepted', 'rejected', 'completed']:

        # BLOCK COMPLETION BEFORE EVENT TIME
        if status == 'completed' and booking.event_date > timezone.now():

            messages.error(
                request,
                "Access Blocked: You cannot finish this job before the scheduled event date and time!"
            )
        ...
        if status == 'accepted':
            ...
            Notification.objects.create(
                user=booking.client,
                message=f"Your booking request with {request.user.username} has been accepted."
            )
            ...
            Notification.objects.create(
                user=booking.client,
                message=f"Your booking request with {request.user.username} was rejected."
            )
            ...
            Notification.objects.create(
                user=booking.client,
                message=f"Your booking with {request.user.username} has been completed."
            )

        messages.success(
            request,
            f"Booking has been marked as {status.title()}!"
        )
