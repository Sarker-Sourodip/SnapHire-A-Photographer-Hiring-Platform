...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Extract the role from cleaned_data
            requested_role = form.cleaned_data['role']
            
            # If a malicious user tries to inject 'developer' via the browser console,
            # we force them to be a 'client' instead.
            ...
            # Pass the sanitized 'final_role' to the service
            user, created = create_user_safely(
                username=form.cleaned_data['username'],
                password=form.cleaned_data['password1'],
                email=form.cleaned_data['email'],
                role=final_role,  # Using the safe variable here
